...
alkaline_earth_metals = [31, 49, 50, 80, 81, 82, 83]

def map_EM_atomic_number(atomic_number):
    if atomic_number in alkaline_earth_metals:
        return alkaline_earth_metals.index(atomic_number) + 1
    else:
        raise ValueError(f"Atomic number {atomic_number} is not in alkaline earth metals range.")

def map_NotEM_atomic_number(atomic_number):
    if 1 <= atomic_number <= 30:
        return atomic_number
    elif 32 <= atomic_number <= 48:
        return atomic_number - 1
    elif 51 <= atomic_number <= 80:
        return atomic_number - 3
    elif 84 <= atomic_number <= 94:
...
class ElementsFeatureProcessor(nn.Module):
    def __init__(self):
        ...
        self.TM_embedding1 = nn.Embedding(8, 8)
        self.NotTM_embedding1 = nn.Embedding(89, 8)
        self.NotTM_embedding2 = nn.Embedding(6, 4)
        
    def forward(self, elements_info, elements_mask, is_em=True):
        ...
        if is_em:
            ...
            for row in atomic_num.view(-1):
                mapped_an.append(map_EM_atomic_number(row.item()) if row>0 else 0)
            ...
            mapped_an = []
            for row in atomic_num.view(-1):
                mapped_an.append(map_NotEM_atomic_number(row.item()) if row>0 else 0)
            mapped_an = torch.tensor(mapped_an, device=atomic_num.device)
            mapped_an = mapped_an.view(atomic_num.shape)

...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, epochs, target_name):
    best_rmse = float('5')
    best_model_path = f'best_{target_name}_{best_rmse}.pth'
    ...
    for epoch in range(epochs):
        ...
        all_preds = []
        all_targets = []
        for ySpec, EMElements, NotEMElements, targets, ySpec_mask, EMElements_mask, NotEMElements_mask in tqdm(train_loader, desc=f'Train Epoch {epoch+1}/{epochs}'):
            ySpec, EMElements, NotEMElements, targets = ySpec.to(device), EMElements.to(device), NotEMElements.to(device), targets.to(device)
            ySpec_mask, EMElements_mask, NotEMElements_mask = ySpec_mask.to(device), EMElements_mask.to(device), NotEMElements_mask.to(device)
            
            optimizer.zero_grad()
            outputs = model(ySpec, ySpec_mask, EMElements, EMElements_mask, NotEMElements, NotEMElements_mask)
        ...
        val_targets = []
        with torch.no_grad():
            for ySpec, EMElements, NotEMElements, targets, ySpec_mask, EMElements_mask, NotEMElements_mask in tqdm(val_loader, desc=f'Val Epoch {epoch+1}/{epochs}'):
                ySpec, EMElements, NotEMElements, targets = ySpec.to(device), EMElements.to(device), NotEMElements.to(device), targets.to(device)
                ySpec_mask, EMElements_mask, NotEMElements_mask = ySpec_mask.to(device), EMElements_mask.to(device), NotEMElements_mask.to(device)
    ...
    test_targets = []
    with torch.no_grad():
        for ySpec, EMElements, NotEMElements, targets, ySpec_mask, EMElements_mask, NotEMElements_mask in tqdm(test_loader, desc=f'Test'):
            ySpec, EMElements, NotEMElements, targets = ySpec.to(device), EMElements.to(device), NotEMElements.to(device), targets.to(device)
            ySpec_mask, EMElements_mask, NotEMElements_mask = ySpec_mask.to(device), EMElements_mask.to(device), NotEMElements_mask.to(device)
            
            outputs, attn_weights = model(ySpec, ySpec_mask, EMElements, EMElements_mask, NotEMElements, NotEMElements_mask, return_attn_weights=True)
...
if __name__ == '__main__':
    ...
    EMElements_info, NotEMElements_info, padded_ySpec, targets, EMLength_max, NotEMLength_max = preprocess_data(df, target_name)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    dataset = MyDataset(EMElements_info, NotEMElements_info, padded_ySpec, targets)

...
alkaline_earth_metals = ['La', 'Ce', 'Pr', 'Nd', 'Pm', 'Sm', 'Eu', 'Gd', 'Tb', 'Dy', 
...
def preprocess_data(filtered_df, target_name):
    ...
    for _, row in filtered_df.iterrows():
        ...
        Elements_list_RM.append(matching_metals)
        Elements_list_notRM.append(non_matching_metals)

    EMLength_max = max(len(i) for i in Elements_list_RM)
    ...
    EMElements_info = [GetElementsInfo(compound, EMLength_max) for compound in Elements_list_RM]
    NotEMLength_max = max(len(i) for i in Elements_list_notRM)
    NotTMElements_info = [GetElementsInfo(compound, NotEMLength_max) for compound in Elements_list_notRM]
...
class MyDataset(Dataset):
    def __init__(self, RMElements_info, NotRMElements_info, padded_ySpec, targets):
        valid_data = [
            (xas, em, nottm, target) 
            for xas, em, nottm, target in zip(padded_ySpec, RMElements_info, NotRMElements_info, targets) 
        ...
        self.RMElements_info = torch.stack([torch.tensor(tm, dtype=torch.float32) for tm in self.RMElements_info])
        self.NotRMElements_info = torch.stack([torch.tensor(nottm, dtype=torch.float32) for nottm in self.NotRMElements_info])
    ...
    def __getitem__(self, idx):
        ...
        RMElements = self.RMElements_info[idx]
        NotRMElements = self.NotRMElements_info[idx]
        target = self.targets[idx]
        ySpec_mask, RMElements_mask, NotRMElements_mask = self.generate_mask(ySpec, RMElements, NotRMElements)
        return ySpec, RMElements, NotRMElements, target, ySpec_mask, RMElements_mask, NotRMElements_mask

...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, epochs, target_name):
    ...
    for epoch in range(epochs):
        ...
        all_preds = []
        all_targets = []
        for ySpec, TMElements, NotTMElements, targets, ySpec_mask, TMElements_mask, NotTMElements_mask in tqdm(train_loader, desc=f'Train Epoch {epoch+1}/{epochs}'):
            ...
            optimizer.zero_grad()
            outputs = model(ySpec, ySpec_mask, TMElements, TMElements_mask, NotTMElements, NotTMElements_mask)
        ...
        val_targets = []
        with torch.no_grad():
            for ySpec, TMElements, NotTMElements, targets, ySpec_mask, TMElements_mask, NotTMElements_mask in tqdm(val_loader, desc=f'Val Epoch {epoch+1}/{epochs}'):
                ySpec, TMElements, NotTMElements, targets = ySpec.to(device), TMElements.to(device), NotTMElements.to(device), targets.to(device)
                ySpec_mask, TMElements_mask, NotTMElements_mask = ySpec_mask.to(device), TMElements_mask.to(device), NotTMElements_mask.to(device)

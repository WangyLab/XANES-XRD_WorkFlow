...
def map_NotTM_atomic_number(atomic_number):
    if 1 <= atomic_number <= 20:
        ...
        raise ValueError(f"Atomic number {atomic_number} is not in Not-TM range.")
...
class ElementsFeatureProcessor(nn.Module):
    ...
    def __init__(self):
        ...
        self.TM_embedding1 = nn.Embedding(21, 8)     # 对应 map_TM_atomic_number 之后的 id 范围
        self.NotTM_embedding1 = nn.Embedding(75, 8)  # 对应 map_NotTM_atomic_number 之后的 id 范围
        self.NotTM_embedding2 = nn.Embedding(6, 4)   # 假设 element_type 的取值是 0~5 之类
    ...
    def forward(self, elements_info, elements_mask):
        ...
        for i in range(B):
            for j in range(N):
                ...
                # 判断是否 TM
                if (21 <= an <= 30) or (39 <= an <= 48):
                    # TM
                    mapped_an = map_TM_atomic_number(an)
                    # 计算 8维 embedding
                    emb_z = self.TM_embedding1(
                        torch.tensor(mapped_an, device=atomic_num.device).unsqueeze(0)
                    )[0]   # shape(8,)
                    out[i, j, 16:24] = emb_z
                    # 对 TM，最后 4 维可以留零(即 out[i,j,24:] = 0)
                else:
                    # Not TM
                    mapped_an = map_NotTM_atomic_number(an)
                    emb_z = self.NotTM_embedding1(
                        torch.tensor(mapped_an, device=atomic_num.device).unsqueeze(0)
                    )[0]   # shape(8,)
                    emb_t = self.NotTM_embedding2(

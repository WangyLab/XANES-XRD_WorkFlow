...
'''Output: Efermi'''
targets = filtered_df2['efermi'].tolist()
...
def mask_module_inference(model, test_loader, mask_xanes=False, mask_tm=False, mask_nottm=False):            
    # Test
    model.load_state_dict(torch.load("C:\\Users\\wangy\\Desktop\\XANES课题\\代码&结果总结\\仅利用3d4d\\efermi_rmse_1.0498_mae_0.7117_r2_0.8518.pth"))
    model.eval()
    test_preds = []
    test_targets = []
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    train_loader, val_loader, test_loader = dataset_random_split("C:\\Users\\wangy\\Desktop\\XANES课题\\代码&结果总结\\仅利用3d4d\\efermi", dataset, train_size=0.7, val_size=0.15, test_size=0.15, new_split=False)
